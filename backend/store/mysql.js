...
function findId(table, conditions, fields) {
  ...
  return new Promise((resolve, reject) => {
    const sql = `SELECT ${field} FROM ${table} WHERE ${table}.Delete=0 ${createConditions(
      conditions
    )}`;
    connection.query(sql, (err, data) => {
      if (err) {
...
function update(table, data, nameIdTable) {
  return new Promise((resolve, reject) => {
    connection.query(
      `UPDATE  ${table} SET ? WHERE ${nameIdTable}=?`,
      [data, data[nameIdTable]],
      (err, result) => {
        if (err) {
          return reject(err);
        }
...
function remove(table, data, nameIdTable) {
  return new Promise((resolve, reject) => {
    connection.query(
      `UPDATE  ${table} SET  ${table}.Delete=1 WHERE ${nameIdTable}=?`,
      data[nameIdTable],
      (err, result) => {
        if (err) {
          return reject(err);
...
function query(table, query) {
  return new Promise((resolve, reject) => {
    connection.query(
      `SELECT * FROM ${table} WHERE ${table}.Delete=0 AND ?`,
      query,
      (err, res) => {
