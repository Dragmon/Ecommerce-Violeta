...
function subcategoriesApi(app) {
    ...
    router.put('/:Id_Subcategory', async function(req, res, next){
        try {
            const updateIdCategory =  await Controller.upsert({...req.body,Id_Subcategory: req.params.Id_Subcategory})
            res.status(200).json({
    ...
    router.delete('/:Id_Subcategory', async function(req, res, next){
        try {
            const deletedIdCategory =  await Controller.remove(req.params.Id_Subcategory);
            res.status(200).json({
